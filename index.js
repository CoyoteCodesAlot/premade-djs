...
client.on('message', message => {
    ...
    for (var i = 0; i < this.commands.length; i++) {
        ...
        if (this.commands[i].prefix === false) {
            if (message.content.split(' ')[0] == `${this.commands[i].name}`) {
                if (this.commands[i].permission != undefined) {
                    if (!message.member.hasPermission(this.commands[i].permission))
        ...
        if (
            message.content.split(' ')[0] == `${this.prefix}${this.commands[i].name}`
        ) {
            if (this.commands[i].permission != undefined) {
                if (!message.member.hasPermission(this.commands[i].permission))
                    return msg.reply('You do not have permission to use this command');
    ...
    console.log(
        `Command "${message.content.split(' ')[0].substr(1)}" not found`.red
    );
});
